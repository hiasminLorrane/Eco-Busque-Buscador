            ...
            adListContainer.innerHTML = adsData.map(ad =>
                `<li>
                    ...
                    <a href="${ad.url}" target="_blank" onclick="registerClick(${ad.id})">
                        <h3>${ad.titulo}</h3>
                        <p>${ad.descricao}</p>
                        <p>Cliques: ${ad.cliques || 0}</p>
                    </a>
                </li>`
            ).join('');
        } else {
            adListContainer.innerHTML = '<p>Sem anúncios disponíveis no momento.</p>';
        }
    } catch (error) {
        console.error('Erro ao buscar anúncios:', error);
        document.getElementById('adList').innerHTML = '<p>Erro ao carregar anúncios.</p>';
    }
}
